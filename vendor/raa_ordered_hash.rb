...
class RaaOrderedHash < Hash
	...
	def == hsh2
		return hsh2==self if !hsh2.is_a?(RaaOrderedHash)
  ...
  def next key
    #return the next key in the sequence or the last key if we are at the end
    i = @order.index(key)
    i = key == @order.last ? i - 1 : i
    @order[i+1]
  end

  def prev key
    #return the prev key in the sequence or the first key if we are at the beginning
    i = @order.index(key)
    i = key == @order.first ? 1 : i
    @order[i-1]
  end
